...
<script>
$(function(){
$("#content").load("ajax_buildingtype.php");

//PAGE NUMBER onClick FUNCTION
$(".page").live("click", function(){
var page = $(this).attr("id");
$("#content").load("ajax_buildingtype.php?page="+page);
});
});
</script>
<script>
...
$("#content").load("ajax_buildingtype.php?searchvalue="+searchvalue);
  });
});
</script>
</script>
...
if ($building != "")
{
if(!mysql_query('INSERT INTO building_type (name,created_by)VALUES ("'.$building.'","'.$user_id.'")'))
{
die('Error: ' . mysql_error());
}
echo '<div class="success_message">Building type created successfully</div>';
}
else
{
...
if ($building != "")
{
if(!mysql_query('UPDATE building_type SET name="'.$building.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
{
die('Error: ' . mysql_error());
}
echo '<div class="success_message">Building type updated successfully</div>';
}
else
{
echo '<div class="error_message">The name should not be empty</div>';
...
&nbsp;
<form id='buildingtype' action="<?php echo $_SERVER['PHP_SELF'];?>" onsubmit="return validateForm()"  method='post' accept-charset='UTF-8'>
...
<input type='hidden' name='submitted' id='submitted' value='1'/>
             <!-- The inner table below is a container for form -->
                <table style="font-family:Arial;" width="100%"  cellpadding="10px" class="htmlForm" cellspacing="0" border="0">
                    ...
                    <tr>
                       <td  width="150px"><label style="margin-left:0px;">Building<em style="font-style:normal;color:red;">*</em></label></td>
                        <td>
						<?php if(isset($_GET['id'])){ ?>
							<input type='text' name='building' id='building' class="textbox" value="<?php echo $name; ?>"/>
				
						<?php } else {?>
							<input type='text' name='building' id='building' class="textbox"/>
						<?php }?>
						</td>
                    </tr>
		<tr >
