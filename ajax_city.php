...
$search=$_GET["searchvalue"];
    $q = mysql_query("SELECT a.id  as id ,a.name,b.name as state_name FROM city a, state b where a.state_id=b.id and a.name like '%$search%' ORDER BY id LIMIT $offset, $rowsperpage");
} else {
    $q = mysql_query("SELECT a.id  as id ,a.name,b.name as state_name FROM city a, state b where a.state_id=b.id ORDER BY id LIMIT $offset, $rowsperpage");
}
...
if($_GET["searchvalue"]){
$search=$_GET["searchvalue"];
	 $total_q = mysql_query("SELECT a.id  as id ,a.name,b.name as state_name FROM city a, state b where a.state_id=b.id and a.name like '%$search%'"); //FOR THE ALL RESULTS
} else {
    $total_q = mysql_query("SELECT a.id  as id ,a.name,b.name as state_name FROM city a, state b where a.state_id=b.id"); //FOR THE ALL RESULTS
}
...
<tbody>';
    ...
    while($r=mysql_fetch_array($q))
    {
		...
		$state =$r['state_name'];
		$emp_id = $r["id"];
        echo '<tr><td>'.$content.'</td><td>'.$state.'</td><td><a href=city.php?id='.$emp_id.'><img src="images/user_edit.png"/></a></td></tr>';
    ...
    //OTHERWISE...
    header("Location: ajax_city.php"); //WILL REDIRECT TO THE FIRST PAGE OF RESULTS
}
}
else
{
$q_initial = mysql_query("SELECT a.id  as id ,a.name,b.name as state_name FROM city a, state b where a.state_id=b.id");
