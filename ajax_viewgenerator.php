...
$search=$_GET["searchvalue"];
    $q = mysql_query("SELECT * FROM generator where generator_name like '%$search%' ORDER BY id LIMIT $offset, $rowsperpage");
} else {
    $q = mysql_query("SELECT * FROM generator ORDER BY id LIMIT $offset, $rowsperpage");
}
...
if($_GET["searchvalue"]){
$search=$_GET["searchvalue"];
	 $total_q = mysql_query("SELECT * FROM generator where generator_name like '%$search%' "); //FOR THE ALL RESULTS
} else {
    $total_q = mysql_query("SELECT * FROM generator"); //FOR THE ALL RESULTS
}
...
<tbody>';
    ...
    while($r=mysql_fetch_array($q))
    {
		...
		$emp_id = $r["id"];
		$building_code=$r["generator_code"];
		$description=$r["description"];
        echo '<tr><td>'.$content.'</td><td>'.$building_code.'</td><td>'.$description.'</td><td>'.$description.'</td><td><a href=edit_generator.php?id='.$emp_id.'><img src="images/user_edit.png"/></a></td>
		<td><a href=view_generator.php?delete_id='.$emp_id.'&delete=1><img src="images/delete.png" onclick="return show_confirm()"/></a></td></tr>';
    ...
    //OTHERWISE...
    header("Location: ajax_viewgenerator.php"); //WILL REDIRECT TO THE FIRST PAGE OF RESULTS
}
}
else
{
$q_initial = mysql_query("SELECT * FROM generator");
