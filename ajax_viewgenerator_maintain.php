...
$search=$_GET["searchvalue"];
    $q = mysql_query("SELECT a.id, a.status,a.description,b.generator_name,b.generator_code FROM generator_maintain a ,generator b  where  a.generator_code=b.id and a.description like '%$search%' ORDER BY a.id LIMIT $offset, $rowsperpage");
} else {
    $q = mysql_query("SELECT a.id,a.status,a.description,b.generator_name,b.generator_code FROM generator_maintain a ,generator b  where  a.generator_code=b.id ORDER BY a.id LIMIT $offset, $rowsperpage");
}
...
if($_GET["searchvalue"]){
$search=$_GET["searchvalue"];
	 $total_q = mysql_query("SELECT a.id,a.status,a.description,b.generator_name,b.generator_code FROM generator_maintain a ,generator b  where  a.generator_code=b.id and a.description like '%$search%' "); //FOR THE ALL RESULTS
} else {
    $total_q = mysql_query("SELECT a.id,a.status,a.description,b.generator_name,b.generator_code FROM generator_maintain a ,generator b  where  a.generator_code=b.id"); //FOR THE ALL RESULTS
}
...
<tbody>';
    ...
    while($r=mysql_fetch_array($q))
    {
		...
		$status=$r["status"];
        echo '<tr><td>'.$content.'</td><td>'.$generator_code.'</td><td>'.$status.'</td><td>'.$description.'</td><td><a href=edit_generator_maintain.php?id='.$emp_id.'><img src="images/user_edit.png"/></a></td>
		<td><a href=view_generator_maintain.php?delete_id='.$emp_id.'&delete=1><img src="images/delete.png" onclick="return show_confirm()"/></a></td></tr>';
    ...
    //OTHERWISE...
    header("Location: ajax_viewgenerator_maintain.php"); //WILL REDIRECT TO THE FIRST PAGE OF RESULTS
}
}
else
{
$q_initial = mysql_query("SELECT * FROM generator_maintain");
